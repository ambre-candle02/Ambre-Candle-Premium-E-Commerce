...
import { Package, Truck, CheckCircle, DollarSign, Users, TrendingUp, Calendar, RefreshCw, Trash2, Eye, Archive, Search, Filter, ChevronDown, Sparkles, MessageCircle } from 'lucide-react';
...
export default function AdminDashboard() {
    ...
    const loadOrders = async () => {
        // 1. FAST LOAD: Show local data immediately
        ...
        toast(
            (t) => (
                <span style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    Clear all orders from this view?
                    <span style={{ display: 'flex', gap: '8px' }}>
                        <button onClick={async () => { toast.dismiss(t.id); setIsRefreshing(true); localStorage.removeItem('ambre_orders'); localStorage.removeItem('ambre_last_order'); setOrders([]); setIsRefreshing(false); toast.success('Orders cleared from view.'); }} style={{ background: '#ef4444', color: '#fff', border: 'none', borderRadius: '6px', padding: '5px 12px', cursor: 'pointer', fontWeight: 700 }}>Clear</button>
    ...
    // OPTIMIZATION: Memoize filtered orders to prevent heavy layout logic on every tiny state change
    ...
    const stats = useMemo(() => {
        const totalRev = orders.reduce((acc, curr) => acc + (parseFloat(curr.total) || 0), 0);
            ...
            <div className="admin-dashboard-grid">
                ...
                <div className="admin-header-v2" style={{ paddingTop: '20px' }}>
                    <div className="admin-header-title">
                        <span style={{ color: '#d4af37', fontWeight: 'bold', letterSpacing: '2px', textTransform: 'uppercase', fontSize: '0.8rem' }}>Management</span>
                        <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 'clamp(1.6rem, 6vw, 2.2rem)', margin: '5px 0 0', whiteSpace: 'nowrap' }}>Order History</h1>
                    </div>
                            ...
                            <tbody>
                                {filteredOrders.length === 0 ? (
                                    ...
                                    filteredOrders.map((order, i) => (
                                        ...
                                        >
                                            <td className="admin-td-id">#{order.orderId || order.id}</td>
                                            <td className="admin-td-customer">
                                                <div className="customer-name">{order.customer?.firstName || 'Guest'} {order.customer?.lastName || ''}</div>
                                                <div className="customer-email">{order.customer?.email || 'No email provided'}</div>
                                            </td>
                                                ...
                                                <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                                                    ...
                                                    <motion.button
                                                        onClick={async (e) => {
                                                            ...
                                                            toast.success(`Order #${order.orderId || order.id} hidden from dashboard.`);
                                                        }}
                                                        className="admin-view-btn"
