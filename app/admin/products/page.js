...
export default function CatalogManagerPage() {
    ...
    useEffect(() => {
        // FAST LOAD: Use local cache if available
        const cached = localStorage.getItem('ambre_catalog_cache');
        if (cached) {
            try {
    ...
    const fetchProducts = async () => {
        try {
            ...
            // Sort appropriately by id
            productsData.sort((a, b) => (b.id || 0) - (a.id || 0));
            setProducts(productsData);
            localStorage.setItem('ambre_catalog_cache', JSON.stringify(productsData));
        } catch (error) {
            console.error("Error fetching products:", error);
            toast.error("Cloud sync failed. Showing cached data.");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="admin-main-container">
            <header style={{ marginBottom: '30px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '20px', paddingTop: '20px' }}>
                <div>
                    ...
                    <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 'clamp(1.6rem, 6vw, 2.2rem)', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Product Catalog</h1>
                                ...
                                {filteredProducts.map((p) => (
                                    <tr key={p.id} style={{ borderBottom: '1px solid #eee' }}>
                                        ...
                                        <td data-label="Actions" style={{ padding: '15px 20px', verticalAlign: 'middle', textAlign: 'right' }}>
                                            ...
                                            >
                                                <Trash2 size={16} /> <span className="admin-btn-text">Delete</span>
                                            </button>
                                        </td>
                                    </tr>
                                ))}
