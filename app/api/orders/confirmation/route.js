    ...
    try {
        ...
        if (type === 'confirmation' || !type) {
            const adminMailOptions = {
                from: `"Ambre Order System" <${process.env.EMAIL_USER}>`,
                ...
                html: `
                    <div style="font-family: sans-serif; padding: 20px; border: 2px solid #d4af37; border-radius: 10px;">
                        ...
                        <p>A new order has been placed on Ambre Candle.</p>
                        <hr/>
                        <p><strong>Order ID:</strong> #${order.id}</p>
                        ...
                        <hr/>
                        <p>Log in to the <a href="${process.env.NEXT_PUBLIC_BASE_URL || ''}/admin">Admin Dashboard</a> to manage this order.</p>
                    </div>
                `
            };
            await transporter.sendMail(adminMailOptions).catch(err => console.error("Admin Email Notify Failure:", err));
        }
