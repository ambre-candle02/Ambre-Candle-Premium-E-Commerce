...
export default function ProductManager() {
    ...
    const handleUpdate = async () => {
        ...
        setIsSaving(true);
        try {
            // Document ID should prefer the actual doc.id we fetched, 
            // falling back to product.id.toString()
            const docId = editingProduct.id.toString();
    ...
    return (
        <div className="manager-v3">
            <header className="v3-header">
                ...
                <button className="v3-add-btn" onClick={() => {
                                        ...
                                        <div className="form-col">
                                            ...
                                            <div className="v-field">
                                                ...
                                                <textarea 
                                                    ...
                                                    value={editingProduct?.description || editingProduct?.desc || ''} 
                                                    onChange={e => {
                                                        if (editingProduct) {
                                    ...
                                    <motion.button 
                                        ...
                                        onClick={async () => {
                                            if (editingProduct && !showAddModal) {
                                                ...
                                                const { name, price, category, image, stock, description } = editingProduct;
            ...
            <style jsx>{`
                ...
                .v3-search { flex: 1; height: 60px; background: #fff; border: 1.5px solid #d4af37; border-radius: 16px; display: flex; align-items: center; padding: 0 25px; gap: 15px; box-shadow: 0 4px 15px rgba(212,175,55,0.05); }
                .v3-search input { width: 100%; border: none; background: transparent; outline: none; font-size: 1rem; color: #1a1a1a; }
                .v3-add-btn { height: 60px; padding: 0 35px; background: #1a1a1a; color: #fff; border-radius: 16px; border: 1.5px solid #d4af37; display: flex; align-items: center; gap: 12px; font-weight: 800; cursor: pointer; transition: 0.3s; white-space: nowrap; }
                ...
                .master-modal-root {
                    ...
                    pointer-events: auto; /* Ensure clicks are intercepted */
                }
                .master-backdrop {
                    position: absolute;
                    inset: 0;
